<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="民宿分类" prop="minsufenlei">
          <el-select v-model="ruleForm.minsufenlei" placeholder="请选择民宿分类">
            <el-option
                v-for="(item,index) in minsufenleiOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="民宿分类" prop="minsufenlei">
	      <el-input v-model="ruleForm.minsufenlei"
                placeholder="民宿分类" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.minsutupian" label="民宿图片" prop="minsutupian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.minsutupian.split(',')" :src="item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.minsumingcheng = true;
          ...
          if(o=='minsufenlei'){
            this.ruleForm.minsufenlei = obj[o];
	    this.ro.minsufenlei = true;
            continue;
          }
          if(o=='minsutupian'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/minsufenlei/minsufenlei`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.minsufenleiOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
	...
	this.ruleForm.minsujieshao = this.ruleForm.minsujieshao.replace(reg,'../../../springbootr0p7i/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
