<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="民宿分类" prop="minsufenlei">
          <el-input v-model="ruleForm.minsufenlei" 
              placeholder="民宿分类" clearable  :readonly="ro.minsufenlei"></el-input>
        </el-form-item>
        <div v-else>
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `minsufenlei/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.minsufenleiCrossAddOrUpdateFlag = false;
                  this.parent.search();
